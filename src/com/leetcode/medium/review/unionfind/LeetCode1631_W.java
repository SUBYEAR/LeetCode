...
/**
 * 你准备参加一场远足活动。给你一个二维 rows x columns 的地图 heights ，其中 heights[row][col] 表示格
 * 子 (row, col) 的高度。一开始你在最左上角的格子 (0, 0) ，且你希望去最右下角的格子 (rows-1, columns-1) 
...
public class LeetCode1631_W {
    public int minimumEffortPath(int[][] heights) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                int id = i * n + j;
                if (i > 0) { // 注意这里只列举了向左和向上的方向其实就已经包含了全部可能情况
