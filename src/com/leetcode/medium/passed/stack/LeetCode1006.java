 ...
 * 相反，我们设计了一个笨阶乘 clumsy：在整数的递减序列中，我们以一个固定顺序的操作符序列来依次替换原有的乘法操作符：乘法(*)，
 * 除法(/)，加法(+)和减法(-)。
...
public class LeetCode1006 {
    public int clumsy(int N) {
        if (N < 4) return calc(N);

        int remainder = -calc(N % 4);
        double res = N * (N - 1) / (N - 2) + N - 3;
        for (int i = 1; i < (N / 4); i++) {
            res = res + calc(N - 4 * i);
        }
        return (int)res + remainder;
    }
